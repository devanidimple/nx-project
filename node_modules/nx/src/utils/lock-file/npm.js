...
function pruneNpmLockFile(lockFileData, packages, projectName) {
    ...
    if (lockFileData.lockFileMetadata.metadata.lockfileVersion === 1) {
        console.warn(`npm v7 is required to prune lockfile. Please upgrade to npm v7 or run "npm i --package-lock-only" to generate pruned lockfile.
...
function pruneTransitiveDependencies(parentPackages, dependencies, prunedDeps, value, modifier) {
    if (!value.dependencies && !value.peerDependencies) {
        return;
    }
    Object.entries(Object.assign(Object.assign(Object.assign(Object.assign({}, value.dependencies), value.devDependencies), value.peerDependencies), value.optionalDependencies)).forEach(([packageName, version]) => {
            ...
            if (dependency) {
                ...
                if (prunedDeps[packageName][key]) {
                    ...
                    if (!currentMeta.find((p) => p.path === dependency.packageMeta[0].path)) {
                        const packageMeta = setPackageMetaModifiers(packageName, dependency, value, modifier);
                    ...
                    pruneTransitiveDependencies([...parentPackages, packageName], dependencies, prunedDeps, prunedDeps[packageName][key], getModifier(packageMeta));
                }
            }
        }
    });
